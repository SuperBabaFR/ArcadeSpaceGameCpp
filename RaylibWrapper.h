...
namespace raywrp {
	...
	void DrawRectangle(Rect rec, Vec2<float> origin, float rotation, Couleur color);
	void DrawRectangleLinesV(Vec2<float> position, Vec2<float> size, Couleur color);
	void DrawContourCercle(Vec2<float> position, float radius, Couleur color);
	...
	Vec2<float> GetScreenToWorld2DPoint(Vec2<float> point, Cam2D cam);
